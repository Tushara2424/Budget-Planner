...
interface Column {
    id: 'index' | 'description' | 'date' | 'amount';
    label: string;
    minWidth?: number;
    align?: 'left';
...
function createData(
    ...
    return { key: key, index: index, description: description, date: date, amount: amount };
}
...
export default function BillsTable(props: any) {
    ...
    useEffect(() => {
        ...
        onValue(billsRef, (snapshot) => {
            const billsRow = snapshot.val();
            let index = 1;
            let row: Data [] = [];
            for (const id in billsRow) {
                const rowData = billsRow[id];
                ...
                if (ts >= startTs && ts <= endTs) {
                    row.push(createData(Number(id), index++, rowData["description"], rowData["date"], rowData["amount"]));
                }
            }
            setRowsData(row);
    ...
    if (props.data.reportType != "bills-table") {
        return (<></>);
    }
